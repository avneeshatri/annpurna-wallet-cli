...
public class WalletContractService {
	...
	private static void approve(Contract contract ,String ownerWalletId, String spenderWallet,String ammount) throws Exception {
		byte[] signature = CryptoUtil.signWithPrivatekey(ownerWalletId.getBytes(), props.getProperty("user.priv.key.path"));
	...
	private static void allowance(Contract contract ,String ownerWalletId, String spenderWallet) throws Exception {
		byte[] signature = CryptoUtil.signWithPrivatekey(ownerWalletId.getBytes(), props.getProperty("user.priv.key.path"));
